                        ...
                        <MyDataTable 
                            ...
                            <Column field="id" header="#" :sortable="true"></Column> 
                                ...
                                <Column header="Actions" :exportable="false" style="min-width:8rem">
                                    <template #body="slotProps">
                                        <button @click="openEditMenuGroupModal(slotProps.data)" class="btn btn-sm btn-icon      btn-text-secondary rounded-pill btn-icon me-2"><i class="ri-edit-box-line"></i></button>
                                        <button @click="deleteMenuGroup(slotProps.data.id)" class="btn btn-sm btn-icon btn-text-secondary rounded-pill btn-icon"><i class="ri-delete-bin-7-line"></i></button>
                                    </template>
                                </Column>
                        </MyDataTable>
                        </div>
                        ...
                        <div class="row g-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-floating form-floating-outline">
                                    ...
                                    class="form-control" 
                                    id="order" 
                                    v-model="formMenuGroup.order" 
                                    placeholder="Masukkan urutan"
                                    @input="formMenuGroup.order = $event.target.value.replace(/[^0-9]/g, '')"
                                    inputmode="numeric"
                                    pattern="[0-9]*"
                                    required
                                    >
...
const modalDescription = computed(() => isEditMode.value ? 'Silakan ubah data menu group di bawah ini.' : 'Silakan isi form di bawah ini untuk menambahkan menu group baru.');
...
const handleSaveMenuGroup = async () => {
    ...
    try {
        ...
        if (!formMenuGroup.value.name || !formMenuGroup.value.icon) {
            Swal.fire('Validasi', 'Nama dan ikon menu group wajib diisi.', 'warning');
            loading.value = false;
            return;
        }

        if (isEditMode.value) {
            // Cari ID menu group dari form atau selectedMenuGroup
            let menuGroupIdToUpdate = formMenuGroup.value?.id || formMenuGroup.value?.idMenuGroup;
            if (!menuGroupIdToUpdate && selectedMenuGroup.value) {
                menuGroupIdToUpdate = selectedMenuGroup.value.id || selectedMenuGroup.value.idMenuGroup;
            }
            if (!menuGroupIdToUpdate) {
                Swal.fire('Error', 'ID Menu Group tidak ditemukan untuk update.', 'error');
                loading.value = false;
                return;
            }
            url = `${$api.menuGroups()}/${menuGroupIdToUpdate}`;
            console.log('Updating menu group with ID:', menuGroupIdToUpdate, 'URL:', url);
            // Update data
            response = await fetch(url, {
                method: 'PUT',
                body: JSON.stringify({
                    name      : formMenuGroup.value.name,
                    icon      : formMenuGroup.value.icon,
                    order     : formMenuGroup.value.order,
                    jenisMenu : formMenuGroup.value.jenisMenu,
                }),
                headers: {
        ...
        if (response.ok) {
            ...
            if (errorData.errors) {
                ...
                Swal.fire('Gagal', errorData.message || `Gagal ${isEditMode.value ? 'memperbarui' : 'membuat'} menu group`, 'error');
            }
        }
    } catch (error) {
        Swal.fire('Error', error.message || 'Terjadi kesalahan saat menyimpan data menu group.', 'error');
    } finally {
        loading.value = false;
    }
};

// Fungsi untuk menangani event load lazy data dari menu group
const loadLazyData = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Gagal memuat data menu group dengan status: ' + response.status }));
            throw new Error(errorData.message || 'Gagal memuat data menu group');
        }

        const result = await response.json();
        menuGroup.value = result.data || []; 
        ...
        console.error('Error loading lazy data for menu group:', error);
        menuGroup.value = [];
        totalRecords.value = 0;
        Swal.fire('Error', `Tidak dapat memuat data menu group: ${error.message}`, 'error');
    } finally {
        loading.value = false;
    }
};
...
const openAddMenuGroupModal = () => {
    ...
    modalTitle.value = 'Tambah Menu Group';
    modalDescription.value = 'Silakan isi form di bawah ini untuk menambahkan menu group baru.';
    resetParentFormState();
};

async function openEditMenuGroupModal(menuGroupData) {
    ...
    if (modalEl && window.bootstrap) {
        ...
        console.error('MenuGroupModal element tidak ditemukan atau Bootstrap belum dimuat.');
    }
}

const deleteMenuGroup = async (menuGroupId) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            url = `${$api.menuGroups()}/${menuGroupId}`;
            ...
            await Swal.fire({
                title: 'Berhasil!',
                text: 'Menu group berhasil dihapus.',
                icon: 'success'
            });
